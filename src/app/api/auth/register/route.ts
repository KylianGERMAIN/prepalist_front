...
import { ApiError, register, setTokens } from "@/lib/auth";
import { CredentialsError, parseCredentials } from "@/lib/credentials";

export async function POST(req: NextRequest) {
  try {
    const { email, password } = parseCredentials(await req.json().catch(() => null), 8);
    await setTokens(await register(email, password));
