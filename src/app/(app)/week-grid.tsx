  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Semaine du {startLabel}</h1>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[720px] space-y-2">
          <div className="grid grid-cols-7 gap-2">
            {days.map((iso, i) => (
              <div key={iso} className="text-center text-sm font-medium">
                {DAYS[i]} <span className="text-muted-foreground">{iso.slice(8, 10)}</span>
              </div>
            ))}
          </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {days.map((iso) => {
                  ...
                  return slot ? (
                    ...
                    <div key={`${iso}_${moment}`} className="min-h-16 rounded-md border border-dashed opacity-40" />
                  );
                })}
              </div>
            </div>
